...
public class ParkingViolationsProcessor {
	...
	public ParkingViolationsProcessor(String format, String input) {
		...
		//		populationMap = PopulationReader.getPopulationMap(); // need to have population reader first and 
		// and add this function in PopulationReader
	}
	...
	// implementation of problem: total parking fines per capita for each ZIP Code

	public Map<String, Integer>  getAvgFineMap() {
		Map<String, Integer> avgFineMap = new HashMap<>();
		for(Area item: populationMap) { //need poulation map in population processor
			String zipCode = item.getZipCode();
			int population = item.getPopulation();
			int avgFine = 0;
			if(parkingViolationFineMap.containsKey(zipCode)) {
